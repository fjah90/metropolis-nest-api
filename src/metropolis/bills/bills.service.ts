...
export class BillsService {
  ...
  async generatePdfFromJson(jsonData: any, userId?: number): Promise<{ id: number, pdfFileName: string; pdfUrl: string, xmlFileName: string, xmlUrl: string }> {
    try {
      console.log("generatePdfFromJson:", userId)

      const docDefinition: any = await this.billReport(jsonData);

      const { id, pdfFileName, pdfUrl, xmlFileName, xmlUrl } = await this.pdfSigningService.signPdf(docDefinition, jsonData.fileName, userId);
